...
export function CSVPreviewModal({ 
  ...
  const [selectedRows, setSelectedRows] = useState(new Set(selectedProcessIds));
  ...
  const isAllInPageSelected = paginatedRows.every(row => selectedRows.has(getRowKey(row)));
              ...
              <TableBody>
                {paginatedRows.length === 0 ? (
                  ...
                  paginatedRows.map((row, index) => (
                    <TableRow key={index} className={isRowSelected(row) ? 'bg-green-50' : ''}>
