...
function parseISO8601DurationToMinutes(isoDuration) {
...
function formatISOToSQLDate(isoDateString) {
...
function formatISOToSQLTime(isoDateString) {
  if (!isoDateString) return '';
  const date = new Date(isoDateString);
...
function getFirstDayOfMonthYYYYMMDD(dateString) {
  if (!dateString) return '';
  const date = new Date(dateString);
  const year = date.getFullYear();
  const month = (date.getMonth() + 1).toString().padStart(2, '0');
...
function buildTimeEntryParams(params) {
  const {
    usuario,
    fechaInicio,
    horaInicio,
  ...
  const sp = SP_PARAMETERS['spNETTiempos_Alta'];
  const opts = sp.optional;

  return {
    Usured: escapeSQL(usuario),
    Fecha: fechaInicio,
    ...
    Proceso: idProceso,
    pTipoHora: tipoHora,
    pParteSalida: opts.ParteSalida,
    pGastos: opts.Gastos,
    pKms: opts.Kms,
    ClienteComercial: opts.ClienteComercial,
    Comentario: escapeSQL(descripcion),
    pCambio: opts.Cambio,
    pTeleTrabajo: opts.TeleTrabajo,
    ObservacionesCalidad: opts.ObservacionesCalidad,
    Rapport: opts.Rapport,
    RapportCheck: opts.RapportCheck,
    VBPermisoUsured: opts.VBPermisoUsured,
    VBPermisoFechaHora: opts.VBPermisoFechaHora,
    ObservacionesPermiso: opts.ObservacionesPermiso,
    Ticket: opts.Ticket,
    EsTeleTrabajo: opts.EsTeleTrabajo,
    pMarcajeIP_INI: opts.MarcajeIP_INI,
    pMarcajeIP_FIN: opts.MarcajeIP_FIN,
    pObservacionesPseudoMarcaje: opts.ObservacionesPseudoMarcaje,
    pTiempoNoReconocido: opts.TiempoNoReconocido,
    pObservacionesRegistroHorario: opts.ObservacionesRegistroHorario
  };
}

function buildTaskParams(params) {
  const {
    nombre,
    fechaInicio,
    fechaFin,
  ...
  const sp = SP_PARAMETERS['spNETTiempos_Procesos_Mantenimiento'];
  const opts = sp.optional;
  ...
  return {
    pAccion: 'I',
    pProceso: opts.Proceso,
    pNombre: escapeSQL(nombre),
    pFechaIniPrevista: fechaIniPrevista,
    pFechaFinPrevista: fechaFinPrevista,
    pFechaIniReal: opts.FechaIniReal,
    pFechaFinReal: opts.FechaFinReal,
    pTiempoPrevisto: minutos,
    pTecnicoPrev: minutos,
    pObservaciones: opts.Observaciones,
    pRutaDOC: opts.RutaDOC,
    pTipoDeHora: tipoHora,
    pPresencial: presencial,
    pDisponible: disponible,
    pCosteEmpresa: opts.CosteEmpresa,
    pFechaAviso: opts.FechaAviso,
    pHoraAviso: opts.HoraAviso,
    pUsuredAviso: opts.UsuredAviso,
    pUsuredResp: opts.UsuredResp,
    pUsuredRespRev: opts.UsuredRespRev,
    pRecursos: opts.Recursos,
    pDiseno: opts.Diseno,
    pTecnicos: escapeSQL(usuario),
    pIdDpto: opts.IdDpto,
    pFase: faseValue,
    pComentarioOblig: opts.ComentarioOblig,
    pCliente: escapeSQL(cliente),
    pIdDptoClte: opts.IdDptoClte,
    pIdAplicacion: opts.IdAplicacion,
    pFechaEstimacion: fechaEstimacion,
    pTareaTecnica: opts.TareaTecnica,
    pObservacionEstExt: opts.ObservacionEstExt,
    pHito: opts.Hito,
    pDesplazamientoPS: opts.DesplazamientoPS,
    pHerramienta: opts.Herramienta,
    pProtegida: opts.Protegida,
    pFaseAnterior: opts.FaseAnterior
  };
}

function generateSQLStatement(params) {
  const {
    usuario,
    fechaInicio,
    horaInicio,
...
function generateTaskSQLInternal(params) {
  const {
    nombre,
    fechaInicio,
    fechaFin,
...
function adaptClockifyEntry(entry, taskMapping, config) {
...
function generateTimeEntrySQL(entries, taskMapping, config) {
  const sqlStatements = [];
  const errors = [];
  let processed = 0;
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    try {
      if (!taskMapping[entry.task]) {
        errors.push({
          line: lineaNum,
          message: `Tarea no encontrada en mapeo: "${entry.task?.substring(0, 30)}..."`
        });
        continue;
      }

      const idProceso = validarIdProceso(taskMapping[entry.task]);
...
function generateTaskSQL(task) {
  return generateTaskSQLInternal(task);
}

function generateFromSource(source, taskMapping, config) {
...
function generateSQLFromObjects(params) {
...
function downloadSQL(sql, filename = 'tiempos.sql') {
...
function formatSQLForHighlight(sql) {
  return sql
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
export {
  ...
  generateFromSource,
  generateSQL,
  generateSQLFromObjects,
  generateSQLStatement,
  generateTaskSQLInternal,
  adaptClockifyEntry,
